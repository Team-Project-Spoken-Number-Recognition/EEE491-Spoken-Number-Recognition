...
| REQ-DEBUG-010 | The UART shall interface with the on-board FT2232HQ USB-UART bridge (FPGA TX → FT2232 RXD). | Board wiring | DBG §1 ¶5 figure | I, H | IMPLEMENTED — NOT VERIFIED (`txd_out` → A18 in `top_debug_demo.xdc`, pin checked against the official XDC; HW-DEBUG-01 open) | Eren |
...
| REQ-HW-002 | FPGA pin assignment shall come from Digilent's `Basys3_Master.xdc`, with port names renamed to the top-level names. | Correct pinning | DBG §3 | I, H | IMPLEMENTED — NOT VERIFIED (`fpga/constraints/top_debug_demo.xdc` derived from the official `Basys3_Master.xdc`; inspection in PR #7, HW-DEBUG-01 open) | Eren |
